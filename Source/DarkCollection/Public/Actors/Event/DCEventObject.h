...
#include "DCEventObject.generated.h"
...
struct FActorsToInteract
{
	...
	float DelayBeforeEachRepeat = 0.1f;
};
...
struct FTransformActorDetails
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	UCurveFloat* TransformCurveFloat; 
...
struct FSpawnActorsAttachment
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FName ParentComponentByTag = "None"; 
...
*/
UCLASS()
class DARKCOLLECTION_API ADCEventObject : public AActor
{
	GENERATED_BODY()
...
private:
	...
	// Environment Event Functions
	void ExecuteTransformEvents(FEvents Event);
	void ExecuteTransformEvent(FTransformActor TransformEvent); 
	void ActorTransformSequence(FTransformActor TransformEvent); 

	void ExecuteSpawnActorEvents(FEvents Event); 
	void ExecuteSpawnActorEvent(FSpawnActors SpawnActorEvent); 

	void ExecuteVisualEvents(FEvents Event); 
	void ExecuteVisualEvent(FActorsVisualOperations VisualEvent);
	...
	void ExecuteFearEvents(FEvents& Event); 
	void ExecuteFearEvent(FFearPlayer FearEvent); 

	// Level Event Functions 
	void ExecuteLevelEvents(FEvents& Event); 
	void LoadStream(FLoadStream LoadStream); 
	void UnloadStream(FUnloadStream UnloadStream); 

	// Sound event functions
	void Execute2DSoundEvents(FEvents& Events); 
	void Execute2DSoundEvent(FPlay2DSound SoundEvent); 
	...
	void InitializeLookAtTimeline();
	void KeepLooking(); 

	void InitializeTransformTimeline(); 
	UFUNCTION() void OnTransformActorTimelineUpdate(float Value);
	UFUNCTION() void OnTransformActorTimelineFinished();
...
private:
	...
	UPROPERTY()
	FTimeline ActorTransformTimeline;

	UPROPERTY()
	FTimerHandle DelayTimerHandle;

	FLookAtActor CurrentLookAtActorEvent;
	FTransformActor CurrentTransformEvent; 
	...
	bool bIsTransformTimelineInitialized = false;
	int8 LookAtCounter = 0;
	int8 TransformSequenceCounter = 0; 
};
